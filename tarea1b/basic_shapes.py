...
def createEnemy(image_filename, # las coordenadas puestas eran para el Koopa pero me arrepentí xd
                n,                                  # cantidad de sprites
                x0 = 1/900 * 429, y0 = 1/678 * 541, # posición inicial del sprite
                dx = 1/900 * 16, dy = 1/678 * 16,   # ancho y largo del sprite
                dx0 =1/900 * 2, dy0 = 0):          # distancia para empezar el siguiente sprite
    ...
    for i in range(n):
        vertices = [
        #   positions        texture
            -0.5, -0.5, 0.0, x0 + dx * i + dx0 * i, y0 + dy,
            0.5, -0.5, 0.0,  x0 + dx * (i + 1) + dx0 * i, y0 + dy,
            0.5,  0.5, 0.0,  x0 + dx * (i + 1) + dx0 * i, y0,
            -0.5,  0.5, 0.0, x0 + dx * i + dx0 * i, y0]
    # Defining connections among vertices
    # We have a triangle every 3 indices specified
        indices = [
            0, 1, 2,
        ...
        shapeEnemy = Shape(vertices, indices, textureFileName)
        
        shapes.append(shapeEnemy)
