...
def funcionMagica(shape, wrapMode, filterMode): #Funcion que mezcla el toGPUshape y textureSimpleSetup
    
    image = Image.open(shape[0].textureFileName)
    img_data = np.array(list(image.getdata()), np.uint8)
    gpus = []
    
    for i in range(len(shape)):
    
        vertexData = np.array(shape[i].vertices, dtype=np.float32)
        indices = np.array(shape[i].indices, dtype=np.uint32)

        # Here the new shape will be stored
        gpuShape = GPUShape()

        gpuShape.size = len(shape[i].indices)
        ...
        if shape[i].textureFileName != None:
